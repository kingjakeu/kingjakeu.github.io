...
RESTful한 통신  
리소스는 uri로 나타내고  
행위는 HTTP method로 나타내고  
리소스의 상태를 주고 받는것   

REST의 특징  
Server-client구조  

- 서버는 API를 제공하고, 비지니스 로직을 처리
- 클라이언트는 사용자 정보를 직접 관리 책임

Stateless  

- 서버는 각각의 요청을 별개의 것으로 인식처리
- 클라이언트는 context(클라이언트의 상태) 를 서버에 저장하지 않음

Cacheable  

- 기존 HTTP 캐시 사용
- no-cache = 매요청 마다 ETag검사

Layered System  

- 클라이언트는 REST API Server만 호출
- 서버는 다중 계층으로 구성가능, 로드밸런싱, 프록시, 게이트웨이

Uniform Interface  

- HTTP 표준 프로토콜을 따르는 모든 플랫폼에서 사용 가능

즉, REST를 잘지켜서 Restful한 통신을 하면 HTTP를 따르는 멀티플랫폼간의 일관성 있는 통신이 가능  
...
Restful 하다?  
URI는 자원을 명확하게 인식 할수 있게 표현  
메소드는 리소스에 대한 행위를 일관서 있게 정의  
...
SSL 통신 절차  
클라가 서버에 접속하면 서버 인증서를 받음  
서버인증서 신뢰 여부 체크 후 공개키 추출  
대칭키로 사용할 임의의 메시지를 공개키로 암호화 후 전송  
서버는 개인키로 복호화 후 통신  
...
인터넷 창에 URL을 입력하면 생기는 일  
브라우저가 URL 문법 확인  
DNS 조회  
ARP(Address Resolution Protocol)로 IP와 MAC address 알아냄  
대상과 TCP통신 소켓 열기  
HTTP 프로토콜 요청  
HTTP 응답값 웹브라우저 출력  
  
TCP와 IP의 차이
TCP는 전체 데이터가 잘 전송될 수 있게 데이터의 흐름제어 및 보장  
IP는 데이터를 다른 장소로 정확하게 옮겨주는 역할
...
List와 Set의 차이  
List는 순서대로 그리고 중복을 허용하지만  
Set은 순서가 보장 되지 않고 중복을 허용하지 않음  
...
Map과 List&Set의 차이  
Map은 Key,value 형식으로 데이터를 관리  
Map에서는 Key는 중복을 허용하지 않고, Value의 중복은 허용  
또한 순서를 보장 하지 않음  
...
그래프  
노드와 그 노드를 연결하는 edge를 하나로 모은 자료구조  
무방향 양쪽으로 connected, 방향 한쪽으로 connect  
루트 노드 계념 x, 순환 혹은 비순환, Self loop도 가능  
사이클이 없는 그래프 = 트리  

그래프와 트리의 차이
사이클이 있으면 그래프, 트리는 없음  
한개의 루트 노드 = 트리, 한개의 부모노드 = 트리
트리는 계층 모델, 그래프는 네트워크 모델  
간선의 개수는 트리는 항상 정점개수-1, 그래프는 지맘데로  


## java

JVM 구조
자바파일을 javac를 통해 java byte code(class)로 컴파일  
클래스 로더는 컴파일된 자바 바이트코드를 런타임 데이터 영역에 로드  
실행 엔진은 클래스 로더를 통해 JVM내의 런타임 데이터 영역에 배치된 바이트코드 실행  
...
트랜잭션  
작업의 완전성을 보장하는 것, 완벽하게 처리하거나, 아니면 복구해주는 것  
트랜잭션은 데이터의 정합성을 보장  
락은 동시성을 제어  
...
디자인 패턴
탬플릿  
미리 알고리즘의 골격을 구현, 안에 기능들은 각자  

어댑터  
하나의 구현체를 다른 인터페이스에서도 같이 동작할 수있도록 변환   

팩토리  
클래스의 인터페이스를 만드는 일을 서브 클래스에게 위임  

파사드  
서브 시스템의 일련의 인터페이스를 통합된 인터페이스로 제공  
